...
def setRoom(prevRoom, direction):
    if direction == 'n':
        graph[prevRoom].update({'n': player.currentRoom.id})
        graph[player.currentRoom.id].update({'s': prevRoom})
    elif direction == 's':
        graph[prevRoom].update({'s': player.currentRoom.id})
        graph[player.currentRoom.id].update({'n': prevRoom})
    elif direction == 'e':
        graph[prevRoom].update({'e': player.currentRoom.id})
        graph[player.currentRoom.id].update({'w': prevRoom})
    elif direction == 'w':
...
def reverseDirection(arr):
    path = []
    index = 0
    for k, v in graph[arr[index]].items():
        path.append(k)
        #print(arr[index], k)
...
def dft(starting_rooom):
    ...
    while len(visited) < 12:
        ...
        # If current room has an exit with none
